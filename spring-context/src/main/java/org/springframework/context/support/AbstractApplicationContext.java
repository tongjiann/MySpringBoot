...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
    ...
    public void refresh() throws BeansException, IllegalStateException {
        synchronized (this.startupShutdownMonitor) {
			...
			/*
			  ...
			  设置容器刷新时间
			  设置容器的状态
			  获取Environment对象，并加载当前系统的属性到对象中
			  准备监听器和事件的集合
			 */
            prepareRefresh();
            ...
            try {
				...
				// 观察者模式
                {
                     ...
                     * 监听器接收到具体的事件之后，可以验证匹配是否能处理当前事件，如果可以，直接处理，不过不行，不做任何操作
                     * =========================================================
                     * 实际代码处理：
                     * 1、提前准备好N多个事件
     ...
     * <br>
     * 配置bean工厂的上下文特征，例如classLoader和增强器
     *
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        // 设置几个自动装配的特殊规则，当在进行ioc容器初始化当如果由多个实现，那么就使用指定当对象进行注入
    ...
    protected void initMessageSource() {
        // 获取bean工厂，一股是DefaultListableBeanFactory
        ConfigurableListableBeanFactory beanFactory = getBeanFactory();
        // 首先判断是否己有xml文件定义了id为messageSource的bean对象
    ...
    protected void initApplicationEventMulticaster() {
        // 获取当前bf，一般是DefaultListableBeanFactory
        ConfigurableListableBeanFactory beanFactory = getBeanFactory();
...
//            getApplicationEventMulticaster().addApplicationListener(getBean(listenerBeanName,ApplicationListener.class));
        }

        // Publish early application events now that we finally have a multicaster...
        // 此处先发布早期的监听器集合(prepareRefresh)
